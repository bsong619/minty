...
export async function POST(request: Request) {
  ...
  let imageBase64: string, mimeType: string, backImageBase64: string | undefined;
  try {
  ...
  console.log("Grade API: imageBase64 length =", imageBase64?.length, "hasBack =", !!backImageBase64);
  if (!imageBase64) {
  ...
  contentParts.push({ type: "text", text: GRADE_PROMPT });
