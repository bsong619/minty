...
export default function SettingsScreen() {
  ...
  const handleRestorePurchases = async () => {
    ...
    if (result.status === "success") {
      ...
      Alert.alert("No Purchase Found", result.message ?? "No active subscription was found.");
    }
  };
    ...
    >
      ...
      <Section title="Subscription">
        <Pressable
          ...
          disabled={restoring}
          style={({ pressed }) => ({ flexDirection: "row", alignItems: "center", gap: 12, padding: 14, opacity: pressed ? 0.7 : 1 })}
        >
          <View style={{ width: 38, height: 38, borderRadius: 11, borderCurve: "continuous", backgroundColor: C.white04, justifyContent: "center", alignItems: "center" }}>
            <Text style={{ fontSize: 18 }}>♻️</Text>
          </View>
          <Text style={{ fontSize: 15, color: C.text, flex: 1 }}>Restore Purchases</Text>
          {restoring ? (
