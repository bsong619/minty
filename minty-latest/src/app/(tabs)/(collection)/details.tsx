...
export default function DetailsScreen() {
  ...
  useEffect(() => {
    if (userId) getScannedCardById(cardId).then((c) => { if (c) setCard(c); });
    else getCards().then((cards) => { const found = cards.find((c) => c.id === cardId); if (found) setCard(found); });
  }, [cardId, userId]);
    ...
    >
      ...
      <View style={{ alignItems: "center", gap: 16, paddingVertical: 8 }}>
        <View style={{ borderRadius: 16, borderCurve: "continuous", overflow: "hidden", boxShadow: SHADOW.hero }}>
          <Image
            source={{ uri: card.tcgImageUrl ?? card.imageUri }}
