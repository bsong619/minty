...
export default function ResultsScreen() {
  ...
  useEffect(() => {
    if (userId) getScannedCardById(cardId).then((c) => { if (c) { setCard(c); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); } });
    else getCards().then((cards) => { const found = cards.find((c) => c.id === cardId); if (found) { setCard(found); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); } });
  }, [cardId, userId]);
    ...
    >
      ...
      <View style={{ alignItems: "center", gap: 16, paddingVertical: 8 }}>
        <View style={{ borderRadius: 16, borderCurve: "continuous", overflow: "hidden", boxShadow: SHADOW.hero }}>
          <Image source={{ uri: card.tcgImageUrl ?? card.imageUri }} style={{ width: 200, height: 280, borderRadius: 16 }} contentFit="cover" />
        </View>
